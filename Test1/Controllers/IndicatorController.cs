...
namespace IndInv.Controllers
{
    ...
    public class IndicatorController : Controller
    {
		...
		public JsonResult editObjectives(Int16 mapID)
		{
			...
			{
				if (objectiveMap.Objective != null)
				{
					objectives = Regex.Matches(objectiveMap.Objective, @"\[.*?\]").Cast<Match>().Select(m => m.Value.Substring(1, m.Value.Length - 2)).ToList();
				}
				else
				{
					objectives = Regex.Matches("", @"\[.*?\]").Cast<Match>().Select(m => m.Value.Substring(1, m.Value.Length - 2)).ToList(); ;
				}
			}
        ...
        public ActionResult editInventory(String indicatorID, Int16? analystID, int fiscalYear)
        {
			...
			var viewModel = viewModelItems.Where(x => x.Indicator_N_Value != true).OrderBy(x => x.Indicator_ID).Select(x => new InventoryViewModel
			{
				Indicator_ID = x.Indicator_ID,
				Area_ID = x.Area_ID,
				Footnote = Helpers.Functions.getFootnoteString(x.Indicator_Footnote_Map.ToList()),
